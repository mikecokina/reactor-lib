...
class AdaptiveConv2DMod(nn.Module):
    ...
    def forward(
        ...
        fmap = func.conv2d(fmap, weights, padding=padding, groups=b)
...
class Attend(nn.Module):
    ...
    def flash_attn(self, q, k, v):
        ...
        out = func.scaled_dot_product_attention(q, k, v, dropout_p=self.dropout if self.training else 0.0)
        return out
...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        return func.normalize(x, dim=1) * self.g * (x.shape[1] ** 0.5)
...
class ResnetBlock(nn.Module):
    def __init__(self, dim, dim_out, *, groups=8, num_conv_kernels=0, style_dims: List = None):
...
class NearestNeighborhoodUpsample(nn.Module):
    ...
    def forward(self, x):
        if x.shape[0] >= 64:
            x = x.contiguous()
        x = func.interpolate(x, scale_factor=2.0, mode="nearest")
        x = self.conv(x)
        return x
...
class EqualLinear(nn.Module):
    ...
    def forward(self, x):
        return func.linear(x, self.weight * self.lr_mul, bias=self.bias * self.lr_mul)
...
class StyleGanNetwork(nn.Module):
    def __init__(self, dim_in=128, dim_out=512, depth=8, lr_mul=0.1, dim_text_latent=0):
        ...
        for i in range(depth):
            ...
            # noinspection PyTypeChecker
            layers.extend([EqualLinear(dim_in_layer, dim_out_layer, lr_mul), nn.LeakyReLU(0.2)])
...
class UnetUpsampler(torch.nn.Module):
    def __init__(
        ...
        full_attention = partial(Transformer, flash_attn=flash_attn)
        *_, mid_dim = up_dims
        ...
        for ind, (
            ...
            attn_klass = full_attention if layer_full_attn else LinearTransformer
    ...
    def resize_image_to(self, x, size):
        return func.interpolate(x, (size, size), mode=self.resize_mode)
    ...
    def forward(
        ...
        h = []
        for blocks, (attn, downsample_) in self.downs:
            for block in blocks:
...
def tile_image(image: Tensor, chunk_size: int = 64) -> Tuple[List[Tensor], int, int]:
    c, h, w = image.shape
...
class AuraSRGigaGAN(object):
    def __init__(
            ...
            model_download_name: str = "AuraSR.safetensors"
    ):
        self._config = {
            "style_network": {
                "dim_in": 128,
        ...
        self.model = UnetUpsampler(**self._config).to(self.device)
        self.input_image_size = self._config['input_image_size']
        ...
        if not os.path.isfile(self.model_file):
            download_model(self.model_file, model_url)

        checkpoint = load_file(self.model_file)
    ...
    @torch.no_grad()
    def upscale_4x_overlapped(
            self,
            image: Image.Image,
            max_batch_size: int = 16,
            weight_type: str = 'checkboard'
    ) -> Image.Image:
        ...
        def process_tiles(tiles: List[Tensor], h_chunks: int, w_chunks: int) -> Tensor:
            num_tiles = len(tiles)
            batches = [
            ...
            for batch in batches:
                model_input = torch.stack(batch).to(self.device)
                generator_output = self.model(
                    lowres_image=model_input,
                    noise=torch.randn(model_input.shape[0], 128, device=self.device),
                )
                reconstructed_tiles.extend(
                    list(generator_output.clamp_(0, 1).detach().cpu())
                )

            return merge_tiles(reconstructed_tiles, h_chunks, w_chunks, self.input_image_size * 4)
